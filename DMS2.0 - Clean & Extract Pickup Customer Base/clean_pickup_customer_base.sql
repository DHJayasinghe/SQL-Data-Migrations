...
SELECT
	...
	REPLACE(T1.Street,'  ','') Street,
	--LEN(REPLACE(Street,'  ','')) StreetLen,
	TRIM(T1.City) City,
	--LEN(TRIM(City)) CityLen,
	T1.HasStreet,
	T1.ORIG_ADDRESS
FROM #tmpSTEP7 T1 INNER JOIN dbo.PICKUP P
ON T1.PICKUP_ID=P.PICKUP_ID
WHERE (LEN(Building)>35 OR LEN(Street)>30 OR LEN(City)>30) AND CONVERT(date,P.CREATED_DATE)>=CONVERT(date,DATEADD(MONTH,-6,GETDATE()))
--SELECT CONVERT(date,DATEADD(MONTH,-6,GETDATE()))
--DROP TABLE #tmpSTEP7
